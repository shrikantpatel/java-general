...
public class Word_Ladder {
    ...
    Map<String, Integer> wordsAlreadyUsed = new ConcurrentHashMap<>();
    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        // if either being or end word not list there is not need to go further
        if (!wordList.contains(endWord)) return 0;

        List<String> nextWordList = findAllWordsWithOneCharDifference(beginWord, wordList);

        int currentLength = 0;
        if (nextWordList.size() <= 0) {
            currentLength = 0;
        } else if (nextWordList.contains(endWord)) {
            currentLength = 1;
        } else {
            for (String nextWord : nextWordList) {
                int childLength = 0;
                if (!wordsAlreadyUsed.containsKey(nextWord)) {
                    List<String> newWordList = new ArrayList<>(wordList);
                    newWordList.remove(nextWord);
                    childLength = ladderLength(nextWord, endWord, newWordList);
                    wordsAlreadyUsed.put(nextWord, childLength);
                } else {
                    childLength = wordsAlreadyUsed.get(nextWord);
                }

                if (currentLength != 0 && childLength != 0) {
                    currentLength = Math.min(currentLength, childLength);
                } else if (currentLength == 0) {
                    currentLength = childLength;
                }
            }
        }
        currentLength = currentLength == 0 ? currentLength : currentLength + 1;
        wordsAlreadyUsed.put(beginWord, currentLength);
        return currentLength;
    }

    public List<String> findAllWordsWithOneCharDifference(String beginWord, List<String> wordList) {
        List<String> nextWordList = new ArrayList<>();
        for (String word : wordList) {
            if (compare(beginWord, word, 1)) {
    ...
    public boolean compare(String word1, String word2, int mistakesAllowed) {
    ...
    @Test
    public void test1() {
        Word_Ladder toTest = new Word_Ladder();
        String beginWord = "hit";
        String endWord = "cog";
        List<String> wordList = Arrays.asList("hot", "dot", "dog", "lot", "log", "cog");
        Assert.assertEquals(5, toTest.ladderLength(beginWord, endWord, wordList));
    }
    ...
    public void test2() {
        ...
        Assert.assertEquals(0, toTest.ladderLength(beginWord, endWord, wordList));
    }
    ...
    public void test3() {
        ...
        String beginWord = "qa";
        String endWord = "sq";
        List<String> wordList = Arrays.asList("si", "go", "se", "cm", "so", "ph", "mt", "db", "mb", "sb", "kr",
                "ln", "tm", "le", "av", "sm", "ar", "ci", "ca", "br", "ti", "ba", "to", "ra", "fa", "yo", "ow", "sn",
                "ya", "cr", "po", "fe", "ho", "ma", "re", "or", "rn", "au", "ur", "rh", "sr", "tc", "lt", "lo", "as",
                "fr", "nb", "yb", "if", "pb", "ge", "th", "pm", "rb", "sh", "co", "ga", "li", "ha", "hz", "no", "bi",
                "di", "hi", "qa", "pi", "os", "uh", "wm", "an", "me", "mo", "na", "la", "st", "er", "sc", "ne", "mn",
    ...
    public void test4() {
        ...
        Assert.assertEquals(3, toTest.ladderLength(beginWord, endWord, wordList));
    }
